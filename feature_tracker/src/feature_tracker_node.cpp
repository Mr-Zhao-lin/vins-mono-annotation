...
FeatureTracker trackerData[NUM_OF_CAM];//该数据类型定义在feature_tracker.h中
 ...
 * @return      void
*/
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)//对于第一帧图像，只记录对应时间戳，不提取特征，因为他没有前一帧图像，无法获取光流。
{
    //判断是否是第一帧
    if(first_image_flag)
    {
   ...
   if (PUB_THIS_FRAME)//发布当前帧，包括id和undistorted后的点，和u,v点
   {
        pub_count++;
