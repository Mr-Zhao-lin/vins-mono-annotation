...
//FeatureTracker的static成员变量n_id初始化为0,检测到0个特征点
...
//图像使用光流法进行特征点跟踪，进行排序并去除密集点.按照被追踪到的次数排序，然后加上mask去掉部分点
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (cur_pts.size() > 0)
    {
        ...
        //将位于图像边界外的点标记为0,清理vector中无法追踪到的特征点
